...
def render_ticket_line(t: dict) -> str:
    if t["kind"] == KIND_REPAIR:
        ...
        reason = f"\nПричина: {t['reason']}" if t["status"] in (STATUS_CANCELED,) and t.get("reason") else ""
...
async def cmd_add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    target = ensure_int(context.args[0])
    if not target:
        await update.message.reply_text("Укажи числовой user_id.")
        return
...
async def cmd_add_tech(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(db, uid):
        ...
        return
    if not context.args:
        await update.message.reply_text("Использование: /add_tech <user_id>")
        return
    target = ensure_int(context.args[0])
    if not target:
        await update.message.reply_text("Укажи числовой user_id.")
