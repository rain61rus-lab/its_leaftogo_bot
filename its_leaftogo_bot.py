...
STATUS_CANCELED = "canceled"   # для покупок может остаться, для ремонтов кнопку убрали
...
def fmt_dt(dt_str: str | None) -> str:
    ...
    try:
        ...
        # Если дата без таймзоны – считаем её локальной
...
def ticket_inline_kb(ticket: dict, is_admin_flag: bool, me_id: int):
    ...
    if ticket["kind"] == KIND_REPAIR:
        ...
        # УБРАНО: кнопка отмены для ремонтов (активна до «выполнена» или «отказ»)
        if (ticket.get("assignee_id") == me_id):
            ...
            # Админ видит, но закрывать не может — только назначение/приоритет
            pass
...
async def cb_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("to_work:"):
        ...
        if t["status"] != STATUS_NEW:
            ...
            return
        # Если заявку создал админ — механик не может брать сам, пока админ не назначит
    ...
    if data.startswith("done:"):
        ...
        if not t or t["kind"] != KIND_REPAIR:
            await query.answer("Некорректная заявка.")
            return
        # Закрыть может ТОЛЬКО исполнитель (админ НЕ может)
        if t.get("assignee_id") != uid:
            await query.answer("Только исполнитель может закрыть заявку.")
            return
        await update_ticket(db, tid, status=STATUS_DONE, done_at=now_local().isoformat())
        await query.edit_message_text((query.message.text or "") + "\n\nСтатус: ✅ Выполнена")
        return
    ...
    # Кнопку «cancel» для ремонтов больше не показываем — блок оставлен для совместимости с покупками
